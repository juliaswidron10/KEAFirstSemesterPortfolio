...
min5.addEventListener("click",switchphoto5);
...
function download(){
    window.open("pdf/Tjubang.pdf");
}
document.querySelector("#storyboard").addEventListener("click",downloadstory);

function downloadstory(){
    window.open("pdf/Storyboard.pdf");
}
...
function switchphoto5(){
    bigpic.classList.remove("pic1");
    bigpic.classList.remove("pic3");
    bigpic.classList.remove("pic4");
    bigpic.classList.remove("pic2");
    bigpic.classList.add("pic5");
}
function showText(){
    if(txtvisible == false){
